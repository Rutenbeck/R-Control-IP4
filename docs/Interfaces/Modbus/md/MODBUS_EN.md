...
Modbus TCP is the language virtually every **PLC** and every **building management system**
understands. The device acts as a **slave** (server): your controller asks, the device answers.

You can switch the four outputs and read the state of outputs, inputs and the temperature
sensor.
...
**Web interface → Settings → Interfaces → Modbus TCP**

| Field | Default |
|---|---|
| Enable Modbus TCP | tick |
| TCP port | `502` |
| Unit ID | `1` |

Do not forget to save. Under **Overview → Device status → Details** the *Modbus TCP* row shows
whether the service is running.

### Step 2: The four most important addresses

| What | Type | Address | Function code |
|---|---|---|---|
| **Switch output 1–4** | Coil | `0`–`3` | 05 (write), 01 (read) |
| **Input 1–4 pressed?** | Discrete Input | `0`–`3` | 02 |
| **Output 1–4 actual state** | Discrete Input | `4`–`7` | 02 |
| **Temperature in °C × 100** | Input Register | `0` | 04 |

> **Watch the numbering:** Addresses are **zero-based**. Output 1 is coil `0`. Some PLC systems
> count from 1 or use prefixes such as `000001` or `40001` — output 1 is then `000001` there.

### Step 3: Try it

With the command line tool `modbus-cli` or any Modbus master:

```
# Switch output 1 on:    FC05, address 0, value 1
# Switch output 1 off:   FC05, address 0, value 0
# Read temperature:      FC04, address 0  →  e.g. 2140 = 21.40 °C
```

The temperature comes as an integer times 100 — divide the value you read by 100.

---

## Common pitfalls

**The output does not switch instantly.** Coil writes are applied to the relays cyclically about
every 100 ms. A delay of a tenth of a second is normal.

**Write `0`/`1` instead of "toggling".** With a PLC a fixed setpoint is always better: after a
connection loss the state is correct again, whereas toggle logic can stay permanently inverted.

**The holding registers are reserved.** They can be read and written but currently carry no
meaning. Do not use them.

**No authentication.** Modbus TCP has no login. Use it only on an isolated plant network.
