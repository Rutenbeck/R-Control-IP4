...
Modbus TCP ist die Sprache, die praktisch jede **SPS** und jede **Gebäudeleittechnik** versteht.
Das Gerät verhält sich dabei als **Slave** (Server): Ihre Steuerung fragt, das Gerät antwortet.

Damit können Sie die vier Ausgänge schalten und den Zustand von Ausgängen, Eingängen und dem
...
**Weboberfläche → Einstellungen → Schnittstellen → Modbus TCP**

| Feld | Standard |
|---|---|
| Modbus TCP aktivieren | Haken setzen |
| TCP Port | `502` |
| Unit ID | `1` |

Speichern nicht vergessen. Unter **Übersicht → Gerätestatus → Details** sehen Sie in der Zeile
*Modbus TCP*, ob der Dienst läuft.

### Schritt 2: Die vier wichtigsten Adressen

| Was | Typ | Adresse | Funktionscode |
|---|---|---|---|
| **Ausgang 1–4 schalten** | Coil | `0`–`3` | 05 (schreiben), 01 (lesen) |
| **Eingang 1–4 gedrückt?** | Discrete Input | `0`–`3` | 02 |
| **Ausgang 1–4 Ist-Zustand** | Discrete Input | `4`–`7` | 02 |
| **Temperatur in °C × 100** | Input Register | `0` | 04 |

> **Achtung, Zählweise:** Die Adressen sind **0-basiert**. Ausgang 1 ist Coil `0`. Manche
> SPS-Systeme zählen ab 1 oder verwenden Präfixe wie `000001` bzw. `40001` — dann ist Ausgang 1
> dort `000001`.

### Schritt 3: Ausprobieren

Mit dem Kommandozeilenwerkzeug `modbus-cli` oder jedem Modbus-Master:

```
# Ausgang 1 einschalten:   FC05, Adresse 0, Wert 1
# Ausgang 1 ausschalten:   FC05, Adresse 0, Wert 0
# Temperatur lesen:        FC04, Adresse 0  →  z. B. 2140 = 21,40 °C
```

Die Temperatur kommt als ganze Zahl mal 100 — teilen Sie den gelesenen Wert durch 100.

---

## Typische Fallstricke

**Ausgang schaltet nicht sofort.** Coil-Schreibzugriffe werden zyklisch etwa alle 100 ms auf die
Relais angewendet. Eine Verzögerung von einem Zehntelsekunde ist normal.

**Schreiben Sie `0`/`1` statt zu „toggeln".** Bei einer SPS ist ein fester Sollwert immer
besser: Nach einem Verbindungsabriss stimmt der Zustand dann wieder, während eine
Toggle-Logik dauerhaft verdreht bleiben kann.

**Die Holding Register sind reserviert.** Sie können gelesen und geschrieben werden, haben aber
derzeit keine Bedeutung. Nutzen Sie sie nicht.

**Keine Authentifizierung.** Modbus TCP kennt keine Anmeldung. Setzen Sie es nur in einem
abgeschotteten Anlagennetz ein.
