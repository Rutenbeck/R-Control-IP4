...
Eine sehr einfache HTTP-Schnittstelle: ein Aufruf im Browser oder per `curl` genügt, um zu
schalten. Kein Anmelden, kein JSON, keine Kopfzeilen.

**Diese Schnittstelle ist für die Rutenbeck Remote App und für bestehende Installationen
gedacht.** Für neue Projekte nehmen Sie besser die [HTTP/JSON-API](../../REST/md/HTTP-JSON-REST-API_DE.md) —
sie ist abgesichert und kann deutlich mehr.

> **Wichtig:** Diese Schnittstelle hat **keine Anmeldung**. Wer das Gerät im Netz erreicht, kann
> schalten. Aktivieren Sie sie nur in einem abgeschotteten Netz.

---

## In 2 Minuten zum Ziel

### Schritt 1: Einschalten

**Weboberfläche → Einstellungen → Schnittstellen → HTTP Legacy API** → Haken setzen, speichern.

### Schritt 2: Schalten

Ausgang 1 umschalten — einfach im Browser aufrufen:

```
http://192.168.0.3/leds.cgi?led=1
```

Jeder Aufruf **schaltet um** (aus → ein → aus). Einen festen Zustand setzen Sie damit nicht;
dafür nehmen Sie die HTTP/JSON-API.

### Schritt 3: Zustand lesen

```
http://192.168.0.3/status.xml
```

Liefert ein kleines XML mit dem Zustand aller Ausgänge und der Temperatur.
...
Gelesen wird das als: Ausgang **1**, Dauer **00:00:03**, Modus **normal** (Impuls EIN).
Mit `reset` statt `normal` erhalten Sie einen Impuls AUS.
