...
A very simple UDP interface: you send a line of text, the device answers with a line of text.
No connection setup, no login.

**This interface exists for existing installations.** For new projects prefer the
[HTTP/JSON API](../../REST/md/HTTP-JSON-REST-API_EN.md) or
[Modbus TCP](../../Modbus/md/MODBUS_EN.md).

> **Important:** No authentication, no encryption. Use only on an isolated network.
...
**Web interface → Settings → Interfaces → UDP Legacy API** → tick, check the port
(default `30303`), save.
...
|---|---|---|
| `OUT1 1` | switch output 1 on | `OK` |
| `OUT1 0` | switch output 1 off | `OK` |
| `OUT1 2` | toggle output 1 | `OK` |
| `OUT1 ?` | read the state | `0` or `1` |
| `T ?` | read the temperature | e.g. `21.40` |
| `OUT1 IMP 00:00:03 1` | impulse ON for 3 seconds | `OK` |

Outputs are **one-based** here: `OUT1` to `OUT4`.
...
$data   = [Text.Encoding]::ASCII.GetBytes("OUT1 1")
$client.Send($data, $data.Length, "192.168.0.3", 30303)
```
