...
Eine sehr einfache UDP-Schnittstelle: Sie schicken eine Textzeile, das Gerät antwortet mit einer
Textzeile. Kein Verbindungsaufbau, kein Anmelden.

**Diese Schnittstelle ist für bestehende Installationen gedacht.** Für neue Projekte nehmen Sie
besser die [HTTP/JSON-API](../../REST/md/HTTP-JSON-REST-API_DE.md) oder
[Modbus TCP](../../Modbus/md/MODBUS_DE.md).

> **Wichtig:** Keine Anmeldung, keine Verschlüsselung. Nur in einem abgeschotteten Netz
> einsetzen.
...
**Weboberfläche → Einstellungen → Schnittstellen → UDP Legacy API** → Haken setzen, Port prüfen
(Standard `30303`), speichern.

### Schritt 2: Die Befehle

| Befehl | Wirkung | Antwort |
|---|---|---|
| `OUT1 1` | Ausgang 1 einschalten | `OK` |
| `OUT1 0` | Ausgang 1 ausschalten | `OK` |
| `OUT1 2` | Ausgang 1 umschalten | `OK` |
| `OUT1 ?` | Zustand abfragen | `0` oder `1` |
| `T ?` | Temperatur abfragen | z. B. `21.40` |
| `OUT1 IMP 00:00:03 1` | Impuls EIN für 3 Sekunden | `OK` |

Die Ausgänge sind hier **1-basiert**: `OUT1` bis `OUT4`.

### Schritt 3: Ausprobieren

Unter Linux/macOS mit `netcat`:

```bash
echo "OUT1 1" | nc -u -w1 192.168.0.3 30303
echo "T ?"    | nc -u -w1 192.168.0.3 30303
```
...
$daten  = [Text.Encoding]::ASCII.GetBytes("OUT1 1")
$client.Send($daten, $daten.Length, "192.168.0.3", 30303)
```
