...
Über MQTT bindet sich das Gerät in eine Hausautomation ein — vor allem in **Home Assistant**.
Das Gerät meldet seinen Zustand an einen MQTT-Broker und nimmt von dort Schaltbefehle entgegen.

Der große Vorteil gegenüber allen anderen Schnittstellen: Home Assistant **legt die Geräte
selbst an**. Sie müssen nichts von Hand konfigurieren — nach dem Einschalten erscheinen vier
Schalter, ein Temperatursensor und die Taster als Auslöser für Automatisierungen.

Sie brauchen dafür einen MQTT-Broker im Netz (z. B. das Mosquitto-Add-on in Home Assistant).

---

## In 5 Minuten zum Ziel

### Schritt 1: Broker-Daten eintragen

**Weboberfläche → Einstellungen → Schnittstellen → Home Assistant MQTT**

| Feld | Beispiel |
|---|---|
| Home Assistant MQTT aktivieren | Haken setzen |
| MQTT Broker URI | `mqtt://192.168.0.50:1883` |
| MQTT Benutzername | `homeassistant` |
| MQTT Passwort | Ihr Broker-Passwort |
| Base Topic | `r-control-ip4` (Standard) |
| Discovery Prefix | `homeassistant` (Standard) |

### Schritt 2: Verbindung prüfen

Drücken Sie **Verbindung testen**. Die Statuszeile sagt sofort, ob Adresse und Zugangsdaten
stimmen — noch bevor Sie speichern.

### Schritt 3: Speichern

Nach dem Speichern verbindet sich das Gerät. Unter **Übersicht → Gerätestatus → Details** steht
in der Zeile *Home Assistant MQTT*, ob die Verbindung steht.

### Schritt 4: In Home Assistant nachsehen

**Einstellungen → Geräte & Dienste → MQTT** → das Gerät erscheint automatisch mit allen
Entitäten. Fertig.

> **Verbindung klappt nicht?** Der Test nennt die Ursache im Klartext, z. B. „Benutzername oder
> Passwort falsch" oder „Broker nicht verfügbar". Sie können die Einstellungen trotzdem
> speichern — das Gerät verbindet sich selbstständig, sobald der Broker erreichbar ist.

---

## Was in Home Assistant erscheint

| Entität | Was Sie damit tun |
|---|---|
| 4 × Schalter | Ausgänge ein- und ausschalten |
| 1 × Temperatursensor | aktuelle Temperatur (wenn NTC angeschlossen) |
| 4 × Geräteauslöser | auf Tastendrücke reagieren (Klick, Doppelklick, langes Drücken) |

Ein Beispiel für eine Automatisierung, die auf einen Doppelklick an Eingang 1 reagiert, legen
Sie in Home Assistant am einfachsten grafisch an: **Automatisierung → Auslöser → Gerät →
R - Control IP 4 → Eingang 1 doppelt geklickt**.

---

> Ab hier folgt der **vollständige Referenzteil** mit allen Details.
