...
Over MQTT the device integrates into a home automation system — above all **Home Assistant**.
The device reports its state to an MQTT broker and accepts switching commands from there.

The big advantage over every other interface: Home Assistant **creates the entities itself**.
You do not have to configure anything by hand — after switching on, four switches, a temperature
sensor and the buttons as automation triggers simply appear.

You need an MQTT broker on your network (e.g. the Mosquitto add-on in Home Assistant).

---

## Getting there in 5 minutes

### Step 1: Enter the broker details

**Web interface → Settings → Interfaces → Home Assistant MQTT**

| Field | Example |
|---|---|
| Enable Home Assistant MQTT | tick |
| MQTT broker URI | `mqtt://192.168.0.50:1883` |
| MQTT username | `homeassistant` |
| MQTT password | your broker password |
| Base topic | `r-control-ip4` (default) |
| Discovery prefix | `homeassistant` (default) |

### Step 2: Check the connection

Press **Test connection**. The status line tells you right away whether address and credentials
are correct — before you even save.

### Step 3: Save

After saving, the device connects. Under **Overview → Device status → Details**, the *Home
Assistant MQTT* row shows whether the connection is up.

### Step 4: Look in Home Assistant

**Settings → Devices & Services → MQTT** → the device appears automatically with all entities.
Done.

> **Connection not working?** The test states the cause in plain text, e.g. "Username or
> password incorrect" or "Broker unavailable". You can still save the settings — the device
> connects on its own as soon as the broker becomes reachable.
...
|---|---|
| 4 × switch | turn outputs on and off |
| 1 × temperature sensor | current temperature (if an NTC is connected) |
| 4 × device trigger | react to button presses (click, double click, long press) |

The easiest way to build an automation reacting to a double click on input 1 is graphically in
Home Assistant: **Automation → Trigger → Device → R - Control IP 4 → Input 1 double clicked**.
