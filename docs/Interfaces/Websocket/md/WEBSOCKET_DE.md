...
Über WebSocket bleibt Ihre Anwendung dauerhaft mit dem Gerät verbunden und erfährt **sofort**,
wenn sich etwas ändert — ohne regelmäßiges Nachfragen. Genau so arbeitet auch die Weboberfläche
des Geräts.

Das ist die richtige Wahl für **Dashboards** und Visualisierungen. Wenn Sie nur gelegentlich
etwas schalten oder abfragen, ist die [HTTP/JSON-API](../../REST/md/HTTP-JSON-REST-API_DE.md)
einfacher.
...
Die Schnittstelle ist immer verfügbar und muss nicht eingeschaltet werden.

### Verbinden und zuhören — ein Beispiel im Browser

```javascript
const ws = new WebSocket("ws://192.168.0.3/ws");

ws.onopen = () => console.log("verbunden");

ws.onmessage = (ereignis) => {
  const nachricht = JSON.parse(ereignis.data);
  console.log(nachricht.type, nachricht);
};
```

Direkt nach dem Verbinden schickt das Gerät einmalig ein `snapshot` mit dem kompletten aktuellen
Zustand. Danach kommen laufend Aktualisierungen: `time` (im Sekundentakt, mit Uhrzeit und
Temperatur), `relay` bei Schaltvorgängen, `button` bei Tastendrücken.

### Einen Ausgang schalten

```javascript
ws.send(JSON.stringify({ type: "relay", index: 0, command: "on" }));
```

`index` ist **0-basiert**: `0` = Ausgang 1.

---

## Gut zu wissen

- **Höchstens 8 gleichzeitige Verbindungen.** Die Weboberfläche belegt selbst eine davon, sobald
  jemand sie geöffnet hat.
- **Verbindungsabbrüche einplanen.** Bauen Sie einen automatischen Neuaufbau ein — nach einem
  Neustart des Geräts oder einer Netzunterbrechung ist die Verbindung weg.
- **`wss://` bei aktiviertem HTTPS**, sonst `ws://`. Der Port ist derselbe wie der des
  Webservers.
