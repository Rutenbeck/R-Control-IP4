...
Over WebSocket your application stays permanently connected to the device and learns
**immediately** when something changes — without polling. This is exactly how the device's own
web interface works.

It is the right choice for **dashboards** and visualisations. If you only switch or query
occasionally, the [HTTP/JSON API](../../REST/md/HTTP-JSON-REST-API_EN.md) is simpler.

---

## Getting there in 5 minutes

The interface is always available and does not need to be enabled.

### Connect and listen — an example in the browser
...
Right after connecting, the device sends a single `snapshot` with the complete current state.
After that, updates keep coming: `time` (every second, with clock and temperature), `relay` on
switching operations, `button` on button presses.
...
`index` is **zero-based**: `0` = output 1.

---

## Good to know

- **At most 8 simultaneous connections.** The web interface occupies one of them as soon as
  someone has it open.
- **Plan for disconnects.** Build in automatic reconnection — after a device restart or a network
  interruption the connection is gone.
- **Use `wss://` when HTTPS is enabled**, otherwise `ws://`. The port is the same as the web
  server's.
